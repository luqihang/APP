...
const SideNav = React.createClass({
    ...
    buildFromSettings() {
        ...
        return this.props.navs.map( navkind => {
            ...
            if ( navkind.navlist ) {
                return (
                  <NavGroup type={this.props.navtype}
                    key={navkind.id} id={navkind.id}   selected={this.props.selected} onClick={this.onSubNavClick} nav={navkind}/>
                );
            } else {
                return (
                  <Nav type={this.props.navtype}
